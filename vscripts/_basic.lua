...
Provides source table values on the iteration step (as second parameter).
...
# diter(source: any | true, iterator = epairs, expand: f(...any): any): iterator | iterator()

Deeply apply the given iterator to an object. (depth-first search)
New created iterator appends current iteration object and level to the end of the source iterator step's parameters.
If first parameter is true, iterator with specified parameters will be returned, not performed.
'expand' function may be provided, witch takes new iterator step's params and returns nested object for iteration. nil means no iteration. By default returns second input param, if it's table.

]]

function diter(v, iterator, expand)
	iterator = iterator or epairs
	expand = expand or function(k, v)
		if type(v) == 'table' then
			return v
		end
	end

	local newiter = function(t)
		local arr = {}
		local lvl = 0

		local function iterate(t)
			lvl = lvl + 1
			local f, o, k = iterator(t)
			while true do
				local params = {f(o, k)}
				k = params[1]
				if k then
					table.insert(params, t)
					table.insert(params, lvl)
					table.insert(arr, params)
					local v = expand(unpack(params))
					if v ~= nil then
						iterate(v)
					end
				else
